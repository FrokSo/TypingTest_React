    ...
    if (numberWordsTyped <= wordDump.length) {
      ...
      for (let i = 0; i < numberWordsTyped; ++i) {
        ...
        for (
          ...
          charIndex < wordDump[i].length;
          ++charIndex
        ) {
          wordSpan.push(
            <span key={`${i}-${charIndex}`} className="spanDefault">
              {wordDump[i][charIndex]}
            </span>
          );
          currentCharIndex++;
        }
        input.push(<span key={i}>{wordSpan}</span>);
  ...
  return (
    <>
      <div className="container" style={{ whiteSpace: "nowrap" }}>
        <div className="referenceText" style={{ whiteSpace: "nowrap" }}>
          {userInput}
        </div>
        <input
